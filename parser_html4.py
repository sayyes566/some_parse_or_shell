...
class MyHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        global g_print, g_mis, g_bool_find_miss, g_tag_start, g_tag_end, g_start, g_line, g_nd, g_def, g_def_find, g_def_pre, g_miss_func
        #print "Start tag:", tag
        if(tag == 'p'): # new function
          g_start = 1
          g_tag_start = 1 
          g_tag_end = 0
          g_bool_find_miss = 0
    ...
    def handle_endtag(self, tag):
        global g_print, g_mis, g_bool_find_miss, g_tag_start, g_tag_end, g_tag
       
        if(tag == 'p'):
            if(g_bool_find_miss == 1 and g_tag_start == 1 and g_tag_end == 0):
                g_mis +=  "\n"
            g_tag_start = 0 
            g_tag_end = 1
    ...
    def handle_data(self, data):
        global g_print, g_mis, g_bool_find_miss, g_tag_start, g_tag_end, g_tag, g_line, g_def_find, g_def, g_def_pre
...
for index in range(len(paths)):
    ...
    for filename in os.listdir(path):
        ...
        if(save_file != ""):
            ...
            with open("./save/compute/"+save_file+".txt", "a") as myfile:
                myfile.write(g_mis.encode('utf8'))
                myfile.close()
            with open("./save/compute/"+save_file+"_func.txt", "a") as myfile:
